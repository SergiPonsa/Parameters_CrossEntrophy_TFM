...
class env_pybullet_kin_gen3() :
    ...
    def __init__(self,visual=True,Excel_path_Okay = "./Original_Mujoco_Training1_averageconverted.xlsx",\
        ...
        #Numpy actions tcp_offset
        self.Actions_offsets_Numpy_files = Actions_offsets_Numpy_files
        self.Actions_offstes_multiplier = 0.05
        ...
        self.parameters_to_modify_base = None
        ...
        self.df_Okay_tcp_list = []
        for Excel in Excels_path_Okay_tcp_list:
            self.df_Okay_tcp_list.append(self.create_df_from_Excel(Excel))
        ...
        for parameter in parameters_to_modify:
            if( parameter in list(self.original_parameters_base_df.columns) ):
                print(parameter+ " okey")
                    ...
                    print("One or more parameters selected are not in the possible ones to modify")
                    print("Possibles to modify")
                    print(list(self.original_parameters_base_df.columns))
                    return
        self.parameters_to_modify_base = parameters_to_modify_base
        self.action_space = len(self.parameters_to_modify)*self.robot.number_robot_control_joints if (parameters_to_modify_base == None)\
                            else len(self.parameters_to_modify)*self.robot.number_robot_control_joints + len(self.parameters_to_modify_base)
